...
from .keras_layers import Lenet, NiN, ED_EMA, CLSTM, PerChannelLSTM, LayerLambdas, ScaledLogReturn
    ...
    def __init__(self, opt):
        ...
        with K.tf.device(device):
            ...
            if opt.use_edema == 1:
                ...
                x = ScaledLogReturn(shape=(opt.height, opt.width, K.int_shape(x)[4]), return_sequences=True)(x)
                x = Lambda(LayerLambdas.OnOffThreshold, LayerLambdas.OnOffThreshold_OutputShape)(x)
            print("Post EMA SHAPE", K.int_shape(x))
